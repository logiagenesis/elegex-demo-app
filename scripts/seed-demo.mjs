...
try {
  ...
  const [[existingJobs]] = await connection.execute("SELECT COUNT(*) AS total FROM jobs WHERE organizationId = ?", [organizationId]);
  if (!existingJobs.total) {
    ...
    for (let monthIndex = 0; monthIndex < 6; monthIndex += 1) {
      const period = new Date(now.getFullYear(), now.getMonth() - 5 + monthIndex, 1, 9, 0, 0);
      const currentMonth = monthIndex === 5; const previousMonth = monthIndex === 4;
      for (let sequence = 0; sequence < 6; sequence += 1) {
        const template = templates[(monthIndex * 2 + sequence) % templates.length];
        const start = new Date(period.getFullYear(), period.getMonth(), 3 + sequence * 4, 8 + (sequence % 3) * 2, 0, 0);
        const end = new Date(start.getTime() + (90 + (sequence % 3) * 30) * 60_000);
        const stage = currentMonth ? ["scheduled", "scheduled", "in_progress", "on_hold", "ready_for_invoicing", "cancelled"][sequence] : previousMonth ? ["invoiced", "invoiced", "ready_for_invoicing", "invoiced", "on_hold", "cancelled"][sequence] : ["invoiced", "invoiced", "invoiced", "invoiced", "invoiced", "cancelled"][sequence];
        const jobNumber = `#${2011 + monthIndex * 6 + sequence}`; const completed = ["ready_for_invoicing", "invoiced"].includes(stage); const contact = contacts[(monthIndex + sequence) % contacts.length]; const foreman = members[(monthIndex + sequence) % members.length] || owner;
        ...
        await connection.execute("INSERT INTO activityLogs (organizationId, actorId, action, entityType, entityId, summary, createdAt) VALUES (?, ?, 'scheduled', 'job', ?, ?, ?), (?, ?, ?, 'job', ?, ?, ?)", [organizationId, owner.id, jobId, `${jobNumber} scheduled with assigned foreman`, new Date(start.getTime() - 2 * 86_400_000), organizationId, foremanId, completed ? "completed" : "updated", jobId, completed ? `${jobNumber} completed with field evidence recorded` : `${jobNumber} remains ${stage.replace(/_/g, " ")}`, end]);
      }
      await connection.execute("INSERT INTO monthlyOperationalSnapshots (organizationId, periodStart, jobsCompleted, jobsInProgress, jobsOnHold, quotesSent, quotesAccepted, invoicesLinked, invoicedValue, firstVisitResolutionRate, customerSatisfactionIndex) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [organizationId, period, 19 + monthIndex * 2, currentMonth ? 6 : 3, 1 + (monthIndex % 3), 9 + monthIndex, 5 + monthIndex, 16 + monthIndex * 2, 128000 + monthIndex * 18500, 82 + (monthIndex % 4), 88 + (monthIndex % 5)]);
    }
    const releases = [["development", "v2.8.0-dev.14", "a12d5f7"], ["staging", "v2.8.0-rc.3", "c8f3a10"], ["production", "v2.7.4", "9db41e2"]];
    for (const [environment, version, commitSha] of releases) { const [release] = await connection.execute("INSERT INTO environmentReleases (organizationId, environment, version, commitSha, status, deployedBy, rollbackVersion, notes) VALUES (?, ?, ?, ?, 'healthy', ?, 'prior-version', 'Synthetic release evidence for the unified demo.')", [organizationId, environment, version, commitSha, owner.id]); if (environment === "staging") for (const [category, checkName] of [["database", "Migration history reviewed"], ["api", "Protected contract suite"], ["ui", "Desktop and mobile smoke path"], ["security", "Tenant scope and RBAC gate"], ["observability", "Database connector health"]]) await connection.execute("INSERT INTO releaseChecks (releaseId, category, checkName, status, evidence) VALUES (?, ?, ?, 'passed', 'Synthetic release evidence seeded for the demonstration workspace.')", [release.insertId, category, checkName]); }
  }
