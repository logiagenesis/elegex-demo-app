...
The unified demo retains its multi-tenant, role-aware, transaction-safe foundations while adding the field-service domain layers shown in the reference. The in-product brand now uses the reference-aligned uppercase **ELEGEX** wordmark with Western Cape context, dashboard terminology is job-centred, synthetic data is explicitly labelled as demonstration data, and repository documentation includes staging, release, operating, and audit evidence. The supporting app icon is deliberately a non-text monogram for favicon and launcher use; it does not replace or reinterpret the verified wordmark.
