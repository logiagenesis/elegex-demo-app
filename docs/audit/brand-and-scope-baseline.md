...
> **Surface verification:** the final SVG mark is used by the shared wordmark component for the dashboard sidebar, signed-out office shell, and sign-in card. The document head uses the same managed SVG for `og:image`, the browser favicon, and the Apple touch icon. The shipped mark is the user-authorized vector reconstruction, not the original opaque checkerboard PNG.
