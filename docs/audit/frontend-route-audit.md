...
The authenticated owner session rendered `/field` as a mobile-first _My field visits_ route. The owner’s assigned scheduled job was visible with job number, field address, contact name, stage, and an active **Open workflow** action. This confirms that the route is backed by the current authenticated user’s tenant-scoped assignment query rather than static mock content. Individual state-changing actions remain subject to the protected foreman API contract and will be covered by their dedicated workflow test and release smoke pass.
