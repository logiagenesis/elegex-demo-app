...
| WP3 — Enforce business rules | PARTIAL / UNVERIFIED REMOTE WORK | `syncLogs` schema; `checkIdempotency` helper; remote `syncQueue` implementation; `docs/audit/python-reviewer-audit.md` | Organization-scoped idempotency keys are now inspected as server-enforced for check-in, material, evidence, quote, and completion. The full consent, no-price, no-delete, time-correction, invoice-link, and completion-exception requirements are not yet verified as server-enforced invariants. |
