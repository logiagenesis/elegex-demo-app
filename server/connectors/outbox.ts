...
export async function enqueueIntegrationEvent(input: {
  ...
  const result = await database.insert(integrationEvents).values({ ...input, availableAt: input.availableAt ?? new Date() }).onDuplicateKeyUpdate({ set: { idempotencyKey: sql`${integrationEvents.idempotencyKey}` } });
  const insertedId = Number(result[0].insertId);
  if (insertedId) return insertedId;
  const existing = await database.select({ id: integrationEvents.id }).from(integrationEvents).where(and(eq(integrationEvents.connectionId, input.connectionId), eq(integrationEvents.idempotencyKey, input.idempotencyKey))).limit(1);
  if (!existing[0]) throw new Error("Idempotent integration event could not be resolved after enqueue");
  return existing[0].id;
}

export async function listDispatchableEvents(organizationId: number, connectionId: number, limit = 25) {
  const database = await getDatabase();
  const connection = await database.select({ id: integrationConnections.id, status: integrationConnections.status }).from(integrationConnections).where(and(eq(integrationConnections.id, connectionId), eq(integrationConnections.organizationId, organizationId))).limit(1);
  if (!connection[0]) throw new Error("Integration connection is not available in this workspace");
  if (connection[0].status !== "active") throw new Error("Integration connection must be active before events can be dispatched");
  return database.select().from(integrationEvents).where(and(eq(integrationEvents.organizationId, organizationId), eq(integrationEvents.connectionId, connectionId), eq(integrationEvents.status, "pending"), lte(integrationEvents.availableAt, new Date()))).orderBy(asc(integrationEvents.availableAt)).limit(Math.min(limit, 100));
}
