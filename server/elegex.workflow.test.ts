...
function configureTransaction(selectResult: unknown[] = []) {
  ...
  mocks.withTransaction.mockImplementation(
    async (work: (transaction: unknown) => Promise<unknown>) => work(tx)
  );
...
describe("critical workflow rollback propagation", () => {
  ...
  it("propagates audit-write failure after task creation through the transaction boundary", async () => {
    ...
    await expect(
      createTask(organizationId, userId, { title: "Prepare handover" })
  ...
  it("propagates audit-write failure after job creation through the transaction boundary", async () => {
    ...
    mocks.values
      ...
      .mockRejectedValueOnce(failure);
    await expect(
      createJob(organizationId, userId, {
        jobNumber: "#9001",
        title: "Safety inspection",
        description: "Inspect the equipment and record a safe handover.",
        contactId: 4,
        serviceAddress: "18 Market Street",
      })
    ).rejects.toBe(failure);
    expect(mocks.withTransaction).toHaveBeenCalledTimes(1);
    expect(mocks.values).toHaveBeenCalledTimes(2);
  ...
  it("propagates audit-write failure after a document record is created through the transaction boundary", async () => {
    ...
    mocks.values
      ...
      .mockRejectedValueOnce(failure);
    await expect(
      createDocumentRecord(organizationId, userId, {
        fileName: "handover.pdf",
        mimeType: "application/pdf",
        sizeBytes: 128,
        storageKey: "elegex/7/documents/handover.pdf",
        storageUrl: "/manus-storage/handover.pdf",
      })
    ).rejects.toBe(failure);
    expect(mocks.withTransaction).toHaveBeenCalledTimes(1);
    expect(mocks.values).toHaveBeenCalledTimes(2);
  ...
  it("propagates stage-change and invoice-link audit failures after dependent writes", async () => {
    ...
    await expect(
      transitionJobStage(organizationId, userId, 88, "in_progress")
    ...
    mocks.limit
      .mockResolvedValueOnce([{ stage: "ready_for_invoicing" }])
      .mockResolvedValueOnce([]);
    const invoiceFailure = new Error("invoice audit insertion failed");
    mocks.values
      .mockResolvedValueOnce([{ insertId: 404 }])
      .mockRejectedValueOnce(invoiceFailure);
    await expect(
      linkExternalInvoice(organizationId, userId, 89, "INV-9001")
    ).rejects.toBe(invoiceFailure);
    expect(mocks.withTransaction).toHaveBeenCalledTimes(1);
    expect(mocks.update).toHaveBeenCalledTimes(1);
    expect(mocks.values).toHaveBeenCalledTimes(2);
  });
  ...
  it("refuses outbox insertion when a tenant-scoped connection is unavailable and propagates a storage failure", async () => {
    ...
    database.select.mockReturnValue({
      from: vi.fn(() => ({
        where: vi.fn(() => ({
          limit: vi.fn().mockResolvedValue([{ id: 9, status: "active" }]),
        })),
      })),
    });
    database.insert.mockReturnValue({
      values: vi.fn(() => ({
        onDuplicateKeyUpdate: vi.fn().mockRejectedValue(outboxFailure),
      })),
    });
    await expect(
      enqueueIntegrationEvent({
  ...
  it("returns the canonical existing event ID on an idempotent outbox replay and refuses paused dispatch", async () => {
    const limit = vi
      .fn()
      .mockResolvedValueOnce([{ id: 9, status: "active" }])
    ...
    const paused = {
      select: vi.fn(() => ({
        from: vi.fn(() => ({
          where: vi.fn(() => ({
            limit: vi.fn().mockResolvedValue([{ id: 9, status: "paused" }]),
          })),
        })),
      })),
    } as any;
    mocks.getDatabase.mockResolvedValue(paused);
    await expect(listDispatchableEvents(organizationId, 9)).rejects.toThrow(
      "must be active before events can be dispatched"
    );
  });
  ...
  it("does not repeat check-in writes when the tenant has already claimed the mobile idempotency key", async () => {
    ...
    await expect(
      foremanCheckIn(
        ...
        userId,
        88,
        "7f5c25f0-6f3b-42c9-8bc1-4392da6e0bc8"
      )
  ...
  it("rejects check-in when the assigned foreman has not captured consent", async () => {
    ...
    mocks.limit
      .mockResolvedValueOnce([
        { id: 88, stage: "scheduled", foremanId: userId },
      ])
      .mockResolvedValueOnce([]);

    await expect(foremanCheckIn(organizationId, userId, 88)).rejects.toThrow(
      "Customer consent must be recorded before check-in"
    );
    expect(mocks.update).not.toHaveBeenCalled();
  });
  ...
  it("requires evidence or an explicit exception before completing a field job", async () => {
    ...
    mocks.limit
      .mockResolvedValueOnce([
        { id: 88, stage: "in_progress", foremanId: userId },
      ])
      .mockResolvedValueOnce([]);

    await expect(
      foremanCompleteJob(organizationId, userId, 88)
    ).rejects.toThrow(
      "Capture field evidence or record a completion exception reason"
    );
    expect(mocks.update).not.toHaveBeenCalled();
  });
  ...
  it("does not archive invoice-ready or invoiced jobs", async () => {
    ...
    await expect(archiveJob(organizationId, userId, 88)).rejects.toThrow(
  ...
  it("does not permit a workspace owner membership to be reassigned through the member-role mutation", async () => {
    ...
    await expect(
      updateMemberRole(organizationId, userId, 88, "admin")
    ).rejects.toThrow("owner role cannot be reassigned");
    expect(mocks.update).not.toHaveBeenCalled();
  });
  ...
  it("rejects updates and archive attempts for unavailable active records before false audit writes", async () => {
    configureTransaction([]);
    await expect(
      updateContact(organizationId, userId, 88, { name: "Missing contact" })
    ).rejects.toThrow("Contact is not available");
    await expect(
      archiveRecord(organizationId, userId, "tasks", 89)
  ...
  it("returns a foreman quote acknowledgement without the persisted numeric total", () => {
    expect(publicForemanQuoteResponse(19, "QT-9020", "draft")).toEqual({
      quoteId: 19,
      quoteNumber: "QT-9020",
      status: "draft",
    });
    expect(
      Object.keys(publicForemanQuoteResponse(19, "QT-9020", "draft"))
    ).not.toContain("total");
  });
  ...
  it("enforces bounded time corrections and normalized invoice reference identifiers", () => {
    ...
    expect(() =>
      assertForemanTimeWindow(new Date("2026-08-21T11:59:59.000Z"), now)
    ).toThrow("backdated by more than 24 hours");
    expect(() =>
      assertForemanTimeWindow(new Date("2026-08-22T12:06:00.000Z"), now)
    ).toThrow("more than five minutes in the future");
    expect(normalizeInvoiceReference(" inv-9001 ")).toBe("INV-9001");
    expect(() => normalizeInvoiceReference("??")).toThrow(
      "Invoice reference must contain"
    );
  });
