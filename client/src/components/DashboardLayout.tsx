...
import { Bell, BriefcaseBusiness, ChartNoAxesCombined, FileText, FolderKanban, LayoutDashboard, LogOut, MapPinned, Menu, Settings2, ShieldCheck, UsersRound, CheckSquare, Wrench } from "lucide-react";
    ...
    return (
      <main className="grid min-h-screen lg:grid-cols-[1.1fr_0.9fr] bg-[#F6F8FC] text-[#14213D]">
        <section className="relative hidden overflow-hidden bg-[#12264F] px-12 py-14 text-white lg:flex lg:flex-col lg:justify-between">
          ...
          <div className="relative max-w-xl"><p className="mb-5 text-xs font-semibold tracking-[0.24em] text-[#A8C8FF]">JOB MANAGEMENT & FIELD SERVICE</p><h1 className="text-5xl font-semibold leading-[1.06] tracking-[-0.04em]">Every job, visible from booking to invoice.</h1><p className="mt-6 max-w-lg text-base leading-7 text-[#D4E2FF]">A precise office workspace for dispatch, field evidence, quotes, invoicing readiness, and the accountable work behind every client visit.</p></div>
          <p className="relative text-sm text-[#A8C8FF]">Elegex Western Cape keeps office control and field delivery in the same line of sight.</p>
        </section>
        <section className="flex items-center justify-center px-6 py-12 sm:px-10"><div className="w-full max-w-md rounded-[1.75rem] border border-[#E4EAF4] bg-white p-8 shadow-[0_20px_60px_rgba(24,54,108,0.10)] sm:p-10"><div className="mb-10 text-sm"><ElegexWordmark /></div><p className="text-sm font-semibold tracking-[0.18em] text-[#195FE6]">OFFICE WORKSPACE</p><h2 className="mt-3 text-3xl font-semibold tracking-[-0.04em] text-[#14213D]">Sign in to operations</h2><p className="mt-3 text-sm leading-6 text-[#667085]">Continue securely with your Manus account. Your first sign-in provisions a private field-service demonstration workspace with synthetic, clearly labelled operating history.</p><Button onClick={() => startLogin()} className="mt-8 h-12 w-full rounded-xl bg-[#195FE6] text-sm font-semibold shadow-[0_8px_20px_rgba(25,95,230,0.25)] hover:bg-[#124FC3]">Continue with Manus</Button><div className="mt-8 rounded-xl bg-[#F5F8FF] p-4 text-sm leading-6 text-[#4B5C7A]"><span className="font-semibold text-[#14213D]">Demonstration access</span><br />No shared password is required. Use any Manus account; the app provisions a tenant-isolated synthetic demo workspace with owner, manager, member, and viewer profiles.</div></div></section>
      </main>
    );
  }
