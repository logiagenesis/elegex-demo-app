...
const state = vi.hoisted(() => ({
  auth: { loading: false, user: null as null | { id: number; name: string }, logout: vi.fn() },
  startLogin: vi.fn(),
...
vi.mock("@/lib/trpc", () => ({ trpc: { elegex: { workspace: { current: { useQuery: () => ({ data: state.workspace }) } } } } }));
...
describe("DashboardLayout rendered interactions", () => {
  beforeEach(() => {
    ...
    Object.defineProperty(window, "matchMedia", {
      writable: true,
      value: vi.fn().mockImplementation(query => ({ matches: false, media: query, onchange: null, addEventListener: vi.fn(), removeEventListener: vi.fn(), addListener: vi.fn(), removeListener: vi.fn(), dispatchEvent: vi.fn() })),
    });
  ...
  it("renders a public recovery shell and invokes the real sign-in action", async () => {
    ...
    await userEvent.click(screen.getByRole("button", { name: "Continue with Manus" }));
  ...
  it("renders authenticated navigation and changes the active browser route on user action", async () => {
    ...
    await userEvent.click(screen.getAllByRole("button", { name: "Commercial reporting" })[0]);
  ...
  it("hides privileged controls from a non-administrative workspace role", () => {
    ...
    expect(screen.queryByRole("button", { name: "Staging readiness" })).toBeNull();
  });
});
