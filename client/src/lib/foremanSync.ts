...
export async function submitQueuedForemanMutation(
  transport: ForemanProcedureTransport,
  mutation: Readonly<SyncMutation>
) {
  const payload = mutation.payload as Record<string, any>;
  if (mutation.type === "consent")
    return transport.consent({
      jobId: Number(payload.jobId),
      signerName: String(payload.signerName),
      idempotencyKey: mutation.id,
    });
  if (mutation.type === "checkIn")
    return transport.checkIn({
      jobId: Number(payload.jobId),
      idempotencyKey: mutation.id,
    });
  if (mutation.type === "material")
  ...
  return transport.complete({
    ...
    exceptionReason: payload.exceptionReason
      ? String(payload.exceptionReason)
      : undefined,
    idempotencyKey: mutation.id,
  });
}
