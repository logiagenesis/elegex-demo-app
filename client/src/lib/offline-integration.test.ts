...
describe("Offline Queue & Idempotency Integration", () => {
  it("preserves data consistency across network drops, reconnects, and idempotent replays", async () => {
    ...
    const expectedMaterialStatus = checkInStatus === "failed" ? "failed" : "queued";
    expect(state.find(m => m.id === material.id)?.status).toBe(expectedMaterialStatus);
    ...
    await mockTrpcTransport.material({ jobId: 123, description: "Wire", quantity: 10, unit: "m", idempotencyKey: material.id });
