...
describe("Network Partition Recovery Simulation", () => {
  it("recovers a backlog of dependent mutations across intermittent connectivity drops", async () => {
    ...
    const queue = new SyncQueue({
      ...
      baseRetryDelayMs: 0,
      maxAttempts: 10,
      transport: (mutation) => submitQueuedForemanMutation(mockTrpcTransport, mutation),
    });

    // 1. Start offline, queue a check-in
    isOnline = false;
    const checkIn = await queue.enqueue({ type: "checkIn", payload: { jobId: 404 } });
    await queue.drain();
    
    let state = await queue.list();
    expect(["retrying", "failed"]).toContain(state.find(m => m.id === checkIn.id)?.status);
    ...
    expect(["retrying", "failed"]).toContain(state.find(m => m.id === mat1.id)?.status);
    expect(["retrying", "failed"]).toContain(state.find(m => m.id === mat2.id)?.status);
    expect(["retrying", "failed"]).toContain(state.find(m => m.id === ev.id)?.status);
