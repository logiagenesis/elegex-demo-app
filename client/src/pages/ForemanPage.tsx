...
function FieldCard({
  ...
  return (
    <section
      className={`rounded-2xl border p-5 ${locked ? "border-[#E4EAF4] bg-[#F8FAFD]" : "border-[#D6E4FF] bg-white shadow-[0_8px_24px_rgba(24,54,108,0.05)]"}`}
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          <h2 className="text-sm font-semibold text-[#14213D]">{title}</h2>
          <p className="mt-1 text-xs leading-5 text-[#667085]">{description}</p>
        </div>
        <span
          className={`rounded-full px-2 py-1 text-[10px] font-bold ${locked ? "bg-[#F0F3F8] text-[#7A879E]" : "bg-[#E7F8EF] text-[#167244]"}`}
        >
          {locked ? "CHECK IN FIRST" : "READY"}
        </span>
      </div>
...
function SyncPanel({
  ...
  const tone =
    state === "error"
      ? "border-[#F7C7D0] bg-[#FFF6F7] text-[#B42342]"
      : state === "synced"
        ? "border-[#BCE8CD] bg-[#F2FCF6] text-[#167244]"
        : "border-[#CFE0FF] bg-[#F5F9FF] text-[#195FE6]";
  const icon =
    state === "error" ? (
  ...
  const label =
    state === "error"
      ...
      : state === "syncing"
        ? "SYNCING FIELD ACTION"
        : queueDepth
          ? `${queueDepth} ACTION${queueDepth === 1 ? "" : "S"} QUEUED`
          : state === "synced"
            ? "SYNCED TO WORKSPACE"
            : "READY TO SYNC";
  return (
    <div
      className={`mt-5 flex items-start gap-2 rounded-xl border px-3 py-2.5 text-xs ${tone}`}
    >
      {icon}
      <div>
  ...
  if (!selected)
    return (
      <>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {(rows || []).map(row => (
            <button
              ...
              className="rounded-3xl border border-[#E4EAF4] bg-white p-5 text-left shadow-[0_8px_24px_rgba(24,54,108,0.05)] transition hover:-translate-y-0.5 hover:border-[#B6D4FF]"
            >
              <div className="flex items-start justify-between gap-3">
                <span className="grid h-10 w-10 place-items-center rounded-2xl bg-[#EAF1FF] text-[#195FE6]">
                  <ClipboardCheck className="h-5 w-5" />
                </span>
                <Badge className="border-0 bg-[#F2F6FF] text-[10px] font-bold text-[#195FE6]">
                  {stageLabel(row.job.stage)}
                </Badge>
              </div>
              <p className="mt-5 text-base font-semibold text-[#14213D]">
                {row.job.jobNumber} · {row.job.title}
              </p>
              <p className="mt-2 flex items-start gap-2 text-sm leading-6 text-[#667085]">
                <MapPin className="mt-0.5 h-4 w-4 shrink-0 text-[#195FE6]" />
                {row.job.serviceAddress}
              </p>
              <div className="mt-5 flex items-center justify-between border-t border-[#EEF2F7] pt-4 text-xs font-semibold text-[#50617F]">
        ...
        {!today.data?.length ? (
          <section className="mx-auto mt-16 max-w-md rounded-3xl border border-[#E4EAF4] bg-white p-8 text-center">
            <ClipboardCheck className="mx-auto h-8 w-8 text-[#195FE6]" />
            <h2 className="mt-4 text-lg font-semibold text-[#14213D]">
              No assigned visits
            </h2>
            <p className="mt-2 text-sm leading-6 text-[#667085]">
              When an office dispatcher assigns you a scheduled job, it will
              appear here as a working field workflow.
            </p>
          </section>
        ) : null}
      </>
...
function ForemanJobWorkspace({
  ...
  const [syncDetail, setSyncDetail] = useState(
    "No field actions are pending. Actions are persisted locally before replay."
  );
  ...
  const materialMutation =
    trpc.elegex.fieldService.foreman.material.useMutation();
  ...
  const transport = useMemo(
    () => ({
      consent: consent.mutateAsync,
      checkIn: checkIn.mutateAsync,
      material: materialMutation.mutateAsync,
      evidence: evidence.mutateAsync,
      quote: quote.mutateAsync,
      complete: complete.mutateAsync,
    }),
    [
      consent.mutateAsync,
      checkIn.mutateAsync,
      materialMutation.mutateAsync,
      evidence.mutateAsync,
      quote.mutateAsync,
  ...
  const queue = useMemo(
    () =>
      new SyncQueue({
        databaseName: `elegex-foreman-${job.id}`,
        transport: async mutation => {
          await submitQueuedForemanMutation(transport, mutation);
        },
      }),
    [job.id, transport]
  );
  const refreshQueue = useCallback(async () => {
    ...
    setQueueDepth(
      rows.filter(row => row.status === "queued" || row.status === "retrying")
        .length
    );
    return rows;
  }, [queue]);
  ...
  useEffect(() => {
    void queue
      ...
      .catch(error => {
        setSyncState("error");
        setSyncDetail(
          `Queued field actions could not be recovered: ${error instanceof Error ? error.message : String(error)}`
        );
      });
  ...
  const queueAction = async (
    ...
    if (saved?.status === "succeeded") {
      setSyncState("synced");
      setSyncDetail(
        "Action replayed to the workspace with its device UUID as the idempotency key."
      );
      toast.success("Field action synchronized");
      onRefresh();
      return;
    }
    if (saved?.status === "failed") {
      ...
      setSyncDetail(
        saved.lastError ||
          "The action remains failed in the local queue. Review connectivity and retry by returning online."
      );
      toast.error(saved.lastError || "Field action needs attention");
      return;
    }
    setSyncState("ready");
    setSyncDetail(
      "Action is safely queued locally and will replay when this browser regains connectivity."
    );
    toast.message("Field action queued for replay");
  };
  ...
  return (
    <>
      <button
        onClick={onBack}
        className="mb-5 text-sm font-semibold text-[#50617F] hover:text-[#195FE6]"
      >
        ← My field visits
      </button>
      <header className="rounded-3xl bg-[#12264F] p-6 text-white shadow-[0_18px_44px_rgba(18,38,79,0.18)]">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-[10px] font-bold tracking-[0.18em] text-[#A8C8FF]">
              FOREMAN JOB CARD
            </p>
            <h1 className="mt-2 text-2xl font-semibold">
              {job.jobNumber} · {job.title}
            </h1>
            <p className="mt-3 flex items-start gap-2 text-sm leading-6 text-[#D4E2FF]">
              <MapPin className="mt-0.5 h-4 w-4 shrink-0" />
              {job.serviceAddress} · {row.contact.name}
            </p>
          </div>
          <span className="rounded-full bg-white/10 px-3 py-2 text-xs font-bold text-[#D4E2FF]">
      ...
      <div className="mt-5 grid gap-4 xl:grid-cols-2">
        <FieldCard
          title="Typed consent signature"
          description="A typed-name artifact is stored once, even if a queued replay is retried."
        >
          <div className="flex gap-2">
            <Input
              value={signerName}
              onChange={event => setSignerName(event.target.value)}
              placeholder="Type full signer name"
              className="h-10"
            />
            <Button
              onClick={() =>
                signerName.trim()
                  ? void queueAction("consent", { jobId: job.id, signerName })
                  : toast.error("Enter the signer’s full name")
              }
              disabled={pending}
              className="bg-[#195FE6]"
            >
        ...
        <FieldCard
          title="Check in"
          description="Consent is a server-enforced prerequisite. The queued check-in depends on queued consent when applicable."
        >
          <Button
            onClick={() =>
              void queueAction(
        ...
        <FieldCard
          title="Materials"
          description="Add an auditable field material entry."
          locked={!active}
        >
          <div className="flex flex-wrap gap-2">
            ...
            <Button
              disabled={!active || pending}
              onClick={() =>
                material.trim() && Number(quantity) > 0
                  ? void queueAction(
                      "material",
                      {
                        jobId: job.id,
                  ...
                  : toast.error("Enter material and quantity")
              }
            >
        ...
        <FieldCard
          ...
          description="Capture evidence by mode; the completion gate requires evidence or an explicit exception."
          locked={!active}
        >
          <div className="grid gap-2 sm:grid-cols-[0.55fr_1fr]">
            <select
              ...
              className="h-10 rounded-md border border-[#DFE7F3] bg-white px-3 text-sm text-[#34435F] disabled:bg-[#F5F7FB]"
            >
              <option value="before_photo">Before condition</option>
              <option value="after_photo">After condition</option>
          ...
          <Button
            ...
            onClick={() =>
              evidenceTitle.trim()
                ...
                : toast.error("Enter an evidence title")
            }
        ...
        <FieldCard
          title="Complete and sync"
          description="Close the field visit for invoice readiness. Explain any evidence gap explicitly."
          locked={!active}
        >
          <Textarea
            disabled={!active || pending}
            value={completionException}
            onChange={event => setCompletionException(event.target.value)}
            placeholder="Completion exception (required if no evidence exists)"
          ...
          <Button
            ...
            onClick={() =>
              void queueAction(
                ...
                {
                  jobId: job.id,
                  exceptionReason: completionException.trim() || undefined,
                },
                activeDependencies
              )
            }
            className="w-full bg-[#167244]"
          >
            <CheckCircle2 className="mr-2 h-4 w-4" />
            Complete job for office review
          </Button>
        </FieldCard>
      </div>
    </>
