...
const ForemanPage = lazy(() => import("@/pages/ForemanPage").then(module => ({ default: module.ForemanPage })));
...
class RouteLoadBoundary extends Component<{ children: ReactNode }, { failed: boolean }> {
  ...
  render() {
    if (this.state.failed) return <section className="mx-auto grid min-h-[60vh] max-w-xl place-items-center text-center"><div className="rounded-3xl border border-[#E4EAF4] bg-white p-8 shadow-[0_16px_45px_rgba(24,54,108,0.08)]"><p className="text-xs font-bold tracking-[0.16em] text-[#195FE6]">VIEW UNAVAILABLE</p><h1 className="mt-3 text-2xl font-semibold text-[#14213D]">This workspace module could not load.</h1><p className="mt-3 text-sm leading-6 text-[#667085]">The navigation shell remains available. Return to the command centre or reload the application to request a fresh module bundle.</p><div className="mt-6 flex flex-col justify-center gap-3 sm:flex-row"><button onClick={() => window.location.assign("/")} className="rounded-xl bg-[#195FE6] px-4 py-2.5 text-sm font-semibold text-white">Command centre</button><button onClick={() => window.location.reload()} className="rounded-xl border border-[#D6E2F6] px-4 py-2.5 text-sm font-semibold text-[#195FE6]">Reload app</button></div></div></section>;
...
function Router() {
  ...
  return <DashboardLayout><RouteLoadBoundary key={location}><Suspense fallback={<RouteLoading />}><Switch>
